...
class SearchViewModel @Inject constructor(val searchUSerCase: GetSearchNewsUseCase) : ViewModel() {
    ...
    fun searchNews(query: String) {
        ...
        viewModelScope.launch {
            queryFlow.debounce(300).filter { query -> query.isNotEmpty()&&query.length > 3  }
                .flatMapLatest { queryLatest -> searchUSerCase(queryLatest) }.catch {
                    _UIState.value = UiState.Error(it.message.orEmpty())
                }.collect { articles -> _UIState.value = UiState.Success(data = articles) }
